...
# Add review_period to df_scored for keyword trend extraction in Step 12c
df_scored["review_period"] = (
    pd.to_datetime(df_scored["review_date"]).dt.year.astype(str) + "-" +
    pd.to_datetime(df_scored["review_date"]).dt.month.astype(str).str.zfill(2)
)
...
# ── Step 12b: Review length vs sentiment ────────────────────────
# Buckets reviews into three length segments and computes average sentiment per bucket. 
# Tests whether more detailed reviews (longer text) correlate with stronger positive or negative sentiment.

print("Aggregating sentiment by review length...")

df_scored["review_length_bucket"] = pd.cut(
    df_scored["review_length"],
    bins=[0, 150, 500, float("inf")],
    labels=["Short (< 150 chars)", "Medium (150–500 chars)", "Long (500+ chars)"]
)

df_review_length = df_scored.groupby(
    ["game_name", "review_length_bucket"],
    as_index=False,
    observed=True
).agg(
    avg_sentiment  = ("sentiment_signed", "mean"),
...
df_review_length["review_length_bucket"] = df_review_length["review_length_bucket"].astype(str)
print(f"  Review length buckets: {len(df_review_length)}")


# ── Step 12c: Keyword trends over time ──────────────────────────
# Runs TF-IDF keyword extraction separately for each month
# This allows us to see how keywords have evolved overtime and patches by developer.
# For each month we extract the top 10 positive and top 10 negative

print("Extracting keyword trends over time...")

from sklearn.feature_extraction.text import TfidfVectorizer

keyword_trends = []

# Get all unique periods in the dataset
periods = df_scored["review_period"].dropna().unique()

for period in sorted(periods):
    df_period = df_scored[df_scored["review_period"] == period]

    # Skip periods with too few reviews to extract meaningful keywords
    if len(df_period) < 50:
        print(f"  Skipping {period} — fewer than 50 reviews")
        continue

    for sentiment in ["POSITIVE", "NEGATIVE"]:
        df_sent = df_period[df_period["sentiment_label"] == sentiment]

        # Need at least 20 reviews per sentiment per period
        if len(df_sent) < 20:
            continue

        try:
            # Fit a fresh TF-IDF vectorizer on this period's reviews
            ...
            vectorizer.fit(df_sent["review"].fillna(""))

            # Get feature names and their mean TF-IDF scores
            tfidf_matrix  = vectorizer.transform(df_sent["review"].fillna(""))
            mean_scores   = tfidf_matrix.mean(axis=0).A1
            feature_names = vectorizer.get_feature_names_out()

            # Take top 10 keywords for this period and sentiment
            top_indices = mean_scores.argsort()[-10:][::-1]

            for idx in top_indices:
                keyword_trends.append({
                    "game_name":       df_scored["game_name"].iloc[0],
                    "review_period":   period,
                    "sentiment":       sentiment,
                    "word":            feature_names[idx],
                    "importance_score": float(mean_scores[idx])
                })

        except Exception as e:
            print(f"  Skipping {period} {sentiment}: {e}")
            continue

df_keyword_trends = pd.DataFrame(keyword_trends)
print(f"  Keyword trend rows: {len(df_keyword_trends)}")
...
print(df_review_length)

print(f"\nKeyword trends over time: {len(df_keyword_trends)} rows across {df_keyword_trends['review_period'].nunique()} periods")
...
spark.createDataFrame(df_review_length).write \
    .format("delta") \
    .mode("overwrite") \
    .option("overwriteSchema", "true") \
    .save(f"{BASE_PATH}/sentiment_by_review_length")

spark.createDataFrame(df_keyword_trends).write \
    .format("delta") \
    .mode("overwrite") \
    .option("overwriteSchema", "true") \
    .save(f"{BASE_PATH}/keyword_trends")
...
df_review_length.to_csv(f"{EXPORT_PATH}/sentiment_by_review_length.csv", index=False)
df_keyword_trends.to_csv(f"{EXPORT_PATH}/keyword_trends.csv", index=False)
